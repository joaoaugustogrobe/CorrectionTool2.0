...
class DockerClient {
	...
	executarTeste(parametros) {
		...
		const { nomeFuncao, input, materia, exercicio, aluno, resolucao, ext } = parametros;
		//funcao: newton
		...
		return new Promise(async (resolve, reject) => {
			//download file
			const res = await this.client.getPlain(`resolucao/${resolucao}/download`);
			console.log('Downloading res:', res.ok);

			try {
				const path = Storage.gerarDiretorio(materia, exercicio, aluno);
				Storage.salvarPlain(path, nomeFuncao + ext, res.data);
			} catch (e) {
				reject(e);
			}
			...
			docker.run(
				...
				stream.on('close', () => {
					...
					resolve({ output: output.replace(expr, ''), _correcao_final, _correcao_inicio })
				})
				// resolve();
			});
